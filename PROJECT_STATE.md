...
- Подготовительный PDF-слой: `file_pdf`, `convert_docx_to_pdf`, UI-кнопка формирования PDF, `PDF_CONVERTER_ENABLED`, `SOFFICE_PATH`; конвертер по умолчанию выключен.
- FAQ: файл `FAQ.md`, публичная страница `/faq/` и ссылка `FAQ` в меню. FAQ доступен всем, чтобы помощь была видна до входа.

## Частично реализовано
- PDF: кнопка и сервисная заготовка реализованы, есть тесты через mock, но PDF-конвертер выключен по умолчанию и требует LibreOffice/`soffice`; production-ready PDF-сценарий остаётся pending.
...
- Production-ready PDF-сценарий — отдельный PR после решения, нужен ли PDF в пилоте; текущая кнопка показывает понятную ошибку, если конвертер выключен или `soffice` недоступен.
...
- PDF-кнопка есть в карточке наряда для сформированных DOCX, но конвертер зависит от LibreOffice/`soffice` и выключен по умолчанию через `PDF_CONVERTER_ENABLED=0`.
