...
DOCX-генерация выполняется через `docxtpl` и сервис `generate_permit_docx`. Для новых русскоязычных шаблонов рекомендуется использовать русские переменные верхнего уровня:
...
{{ дополнительные_меры_безопасности }}
{{ дата_начала }}
{{ дата_окончания }}
{{ дата_создания }}
{{ ответственный_руководитель }}
{{ производитель_работ }}
{{ создал_пользователь }}
{{ опасности }}
{{ меры_безопасности }}
...
Существующие английские переменные вида `{{ permit.number }}` и `{{ permit.work_location }}` остаются совместимыми, чтобы старые шаблоны не ломались. Даты в русских алиасах отдаются в формате `ДД.ММ.ГГГГ`, списки опасностей и мер безопасности — готовым текстом через запятую, пустые значения — пустой строкой. Подробная карта переменных находится в `DOCX_TEMPLATE_MAPPING.md`.

В наряде можно сочетать справочники и ручные значения: `WorkArea`, `Equipment`, `WorkType`, `Hazard` и `SafetyMeasure` остаются типовыми справочниками, а поля ручного ввода позволяют указать нестандартного ответственного, производителя работ, характер работ, дополнительные условия и дополнительные меры безопасности. Для проверки шаблона без реального наряда в Django admin у `DocumentTemplate` есть кнопка **«Проверить шаблон»**: она генерирует тестовый DOCX на demo-данных и не создаёт `GeneratedDocument`.
